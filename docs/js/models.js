...
export function initConsultModel() {
    ...
    mtlLoader.load("./models/consult.mtl", (materials) => {
        ...
        objLoader.load("./models/consult.obj", (obj) => {
            ...
            // 🔹 Atualizar materiais existentes (mantém textura, adiciona efeito vidro)
            obj.traverse((child) => {
                if (child.isMesh && child.material) {
                    let baseMap = child.material.map || null; // textura original, se existir

                    child.material = new THREE.MeshPhysicalMaterial({
                        map: baseMap,                          // mantém textura do .mtl
                        color: new THREE.Color(0xffffff),      // preserva as cores originais
                        metalness: 0.1,
                        roughness: 0.25,
                        transmission: 0.4,                     // 🔹 menos transparente
                        opacity: 10,                         // 🔹 mais visível
                        transparent: true,
                        thickness: 0.5,
                        ior: 1.3,
                        reflectivity: 0.3,                     // menos reflexo
                        clearcoat: 0.6,
                        clearcoatRoughness: 0.2,
                        envMap: envTexture,
                        envMapIntensity: 0.5,                  // 🔹 reduz reflexo do ambiente
                        side: THREE.FrontSide,
                    });
                }
            });

            // 🔹 Criar borda roxa translúcida (vidro roxo)
            obj.traverse((child) => {
                if (child.isMesh) {
